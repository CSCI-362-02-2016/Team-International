...
for vm_opts_file in "$IDE_BIN_HOME/studio$BITS.vmoptions" "$OS_SPECIFIC_BIN_DIR/studio$BITS.vmoptions" "$HOME/.AndroidStudio2.1/studio$BITS.vmoptions" "$STUDIO_VM_OPTIONS"; do
...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$IDE_BIN_HOME/lib$AGENT_LIB.so" ]; then
    AGENT="-agentlib:$AGENT_LIB=disablealloc,delay=10000,sessionname=AndroidStudio2.1"
  fi
fi
...
LD_LIBRARY_PATH="$IDE_BIN_HOME:$LD_LIBRARY_PATH" "$JAVA_BIN" \
  ...
  -Djb.restart.code=88 -Didea.paths.selector=AndroidStudio2.1 \
...
RESTARTER="$HOME/.AndroidStudio2.1/restart/restarter.sh"
if [ -x "$RESTARTER" ]; then
  "$RESTARTER"
  "$RM" -f "$RESTARTER"
fi
